...
| --- | --- | --- |
| macOS Apple Silicon | `Aegis-<version>-macos-arm64.pkg` | Installs `Aegis.app`, `aegis`, and `aegis-server`. DMG is still available for drag-install diagnostics. |
| Windows x64 | signed `Aegis-<version>-windows-x64.msix` | Primary Windows installer. Portable ZIP builds are dev-artifact diagnostics only, not public install assets. |
...
Do not hand-edit generated release assets here. PKG, DMG, Windows MSIX, APK,
AAB, IPA, and `SHA256SUMS` should be produced from the private Aegis source tree,
