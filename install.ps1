...
$StateDir = Join-Path $AegisHome ".aegis"
...
function Write-Env {
  New-Item -ItemType Directory -Force -Path $AegisHome, $StateDir | Out-Null
  if ($WorkerOnly) {
  ...
  Set-Content -Path (Join-Path $AegisHome ".env") -Value @(
    ...
    "AEGIS_SQLITE_PATH=$(Quote-Env (Join-Path $StateDir "aegis.sqlite"))",
